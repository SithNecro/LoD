...
function cambiarImagenSeleccionada() {
    ...
    const audio = new Audio(`img/Evento_Desierto/${urlaudio}`);
    if (ComprobarMute == "on") {
...
function cargarTesoroLegendario() {
    fetch('img/Listado_Cartas.json')
        ...
        .then(data => {
            ...
            // Cambiar la imagen
            const imagen = document.getElementById('imagen-tesoro');
            imagen.src = `img/Evento_Desierto/${tesoroAleatorio}`;
...
function barajarTesoros() {
    document.getElementById('imagen-tesoro').src = 'img/traseras/Trasera Desierto.png';
}
