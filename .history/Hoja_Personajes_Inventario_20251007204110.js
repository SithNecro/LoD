  ...
  await Swal.fire({
    ...
    didOpen: () => {
      ...
      const onClick = async (ev) => {
        ...
        if (action === 'eliminar') {
          // 🔧 Apunta la confirmación al contenedor del popup de inventario
          const invTarget =
            document.getElementById('invRoot') || // si tu popup tiene este id
            document.querySelector('.swal2-popup .sai-body') || // tu contenedor interno
            document.querySelector('.swal2-popup'); // fallback seguro

          const Confirm = Swal.mixin({
            target: invTarget,          // ⬅️ Renderiza la confirmación DENTRO del popup
            backdrop: true,             // backdrop limitado al target
            allowOutsideClick: false,
            allowEscapeKey: false,
            width: '420px',
            customClass: {
              popup: 'sai-popup',       // mantiene tu estilo
            },
          });
          ...
          Confirm.fire({
            ...
            if (delObj || delArm || delArma) {
              ...
              // Mini aviso también dentro del popup, sin cerrarlo
              Confirm.fire({
                title: 'Eliminado',
