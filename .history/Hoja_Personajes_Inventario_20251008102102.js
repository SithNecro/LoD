...
const onClick = async (ev) => {
  ...
  // ¿Viene de menú fijo (fuera de #invRoot)?
  const inFixedMenu = !!btn.closest('.inv-actions-menu-fixed');
  let row = null, id = 0;

  if (inFixedMenu) {
    // id pasado en el propio botón del menú
    id = Number(btn.dataset.itemid || 0);
  } else {
    // flujo habitual dentro de la tabla
    row = btn.closest('[data-itemid]'); if (!row) return;
    id = Number(row.dataset.itemid);
  }

  const action = btn.dataset.action;
  // Si hay un menú abierto y se pulsa una acción, lo cerramos ya
  if (inFixedMenu) {
...
if (btn.dataset.action === 'acciones') {
  ...
  menu.style.cssText = `
    position:fixed;
    z-index:10050; /* por encima de bordes y del contenido del modal */
    ...
    min-width:150px;
  `;
  // ⬇️ OJO: pasamos data-itemid a los botones para que onClick pueda leerlo
  menu.innerHTML = `
  ...
  // Inserta primero para medir y ajustar si se sale
  ...
  // Cierre si clic fuera / scroll / resize
  ...
  return; // dejamos que los botones del menú los gestione el MISMO onClick (ver cambio 2)
}
