...
window.openInventarioEditor = async function (slot) {
      ...
      <div id="invRoot" style="max-height:${modalHeight - 90}px; overflow:auto;">
        ...
        <div class="border rounded p-2 mb-3 hero-card">
          <div class="row g-2 align-items-end">
            ...
            <!-- Grupo Armadura (cobertura con acumulación en <p>) -->
    ...
    await Swal.fire({
        ...
        didOpen: () => {
            ...
            // ===== Exclusividad de ATAJOS (solo en Objetos) =====
            const isAtajo = (v) => v && v.startsWith('Atajo ');
            const findObjetoByAtajo = (atajo, exceptId = null) => {
                return personaje.inventario.objetos.find(o => o.lugar === atajo && o.id !== exceptId) || null;
            };
            const ensureAtajoLibre = (atajo, exceptId = null) => {
                const conflict = findObjetoByAtajo(atajo, exceptId);
                return !conflict;
            };

            // ===== Helpers de limpieza y cobertura ARM =====
            let armCobSel = [];
            ...
            // Inicio: placeholder + grupos ocultos
            resetTipo();
            ...
            document.getElementById('btnAddObj').addEventListener('click', async () => {
                ...
                if (isAtajo(lugar) && !ensureAtajoLibre(lugar)) {
                    await Swal.fire({
                        icon: 'warning',
                        title: 'Atajo ocupado',
                        text: `Ya existe un objeto en "${lugar}". Elige otro lugar.`,
                        customClass: { popup: 'sai-popup', title: 'sai-title', htmlContainer: 'sai-html', actions: 'sai-actions', confirmButton: 'sai-confirm', cancelButton: 'sai-cancel' }
                    });
                    return; // no añadir
                }
            ...
            // ===== Edición inline (incluye exclusividad de Atajos al cambiar Lugar) =====
            const onInput = async (ev) => {
                ...
                const updateItem = (arr, fields) => {
                    ...
                    // Exclusividad: si el campo es 'lugar' en Objetos y es Atajo, comprobar
                    if (arr === personaje.inventario.objetos && f === 'lugar' && isAtajo(v)) {
                        const oldV = arr[idx].lugar || '';
                        if (!ensureAtajoLibre(v, id)) {
                            // revert UI
                            if (t.tagName === 'SELECT') t.value = oldV || '';
                            Swal.fire({
                                icon: 'warning',
                                title: 'Atajo ocupado',
                                text: `El "${v}" ya está asignado a otro objeto.`,
                                customClass: { popup: 'sai-popup', title: 'sai-title', htmlContainer: 'sai-html', actions: 'sai-actions', confirmButton: 'sai-confirm', cancelButton: 'sai-cancel' }
                            });
                            return false; // no actualizar ni guardar
                        }
                    }
