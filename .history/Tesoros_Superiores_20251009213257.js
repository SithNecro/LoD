...
function cargarStatsObjeto(tesoroSeleccionado) {
    fetch('json/stats_Tesoros_Superiores.json')
        .then(r2 => r2.json())
        .then(stats => {
            const detalle2 = document.getElementById('enemigos-lista');
            //detalle2.innerHTML = ``; // limpiar
...
window.tc_parseEnemyItem = function (itemEl) {
   ...
   // Efecto/Leyenda (visible + tooltip, sin recortar)
let efectoTxt = '';
...
if (efectoP && efectoP.parentElement) {
  ...
  if (tip) {
    const visible = tip.textContent.trim(); // lo que se ve, ej. "Apilable"
    const html = tip.getAttribute('data-tippy-content') || '';
    const tmp = document.createElement('div');
    tmp.innerHTML = html;
    const tooltip = tmp.textContent.trim(); // texto interno sin etiquetas
    // 👇 combinamos visible + tooltip
    efectoTxt = `${visible} -> ${tooltip}`;
  } else {
    // fallback: sin tooltip, tomamos todo el texto tras "Efecto:"
    efectoTxt = cont.textContent
      .replace(efectoP.textContent, '')
      .trim()
      .replace(/^[:\s]+/, '')
      .trim();
  }
    ...
    if (categoria === 'objeto') {
        return {
            ...
            item: {
                id: Date.now(),
                nombre: nombreTipo || nombreBase,     // sin .png y sin "(x)"
