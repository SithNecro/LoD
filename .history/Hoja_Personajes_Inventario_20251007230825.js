  ...
  try {
    ...
    const tblObjs = `
      ...
      <table class="table table-sm table-dark table-striped">
        ...
        <th  style="width:40px; text-align:center; vertical-align:middle;" title="Unidades">🧮</th>
        <th  style="width:40px; text-align:center; vertical-align:middle;"title="Peso">⚖️</th>
        ...
        <tbody>
          ${objetosOrden.map(o => `
            <tr data-itemid="${o.id || ''}" data-cat="obj">
              <td style="text-align:center; vertical-align:middle;"><span title="${esc(o.uso || '')}">${esc(o.nombre || '')}</span></td>
              <td   style="width:40px; text-align:center; vertical-align:middle;">${esc(o.cantidad ?? 0)}</td>
              <td style="width:40px; text-align:center; vertical-align:middle;">${esc(o.peso ?? '')}</td>
              <td class="text-center">
                <button class="btn btn-sm btn-outline-warning slot-traspasar" data-cat="obj" data-id="${o.id || ''}" data-slot="${slot}">⇄</button>
              </td>
            </tr>`).join('')}
        </tbody>
      </table>`;
    ...
    const tblArmad = `
      ...
      <table class="table table-sm table-dark table-striped">
        <thead><tr><th>Equip.</th><th>Armadura</th><th>⛇</th><th>🛡️</th><th>Dur.</th><th>⚖️</th><th>⇄</th></tr></thead>
        <tbody>
          ${armadurasOrden.map(a => {
            const dur = parseInt(a.durabilidad ?? 0, 10);
    ...
    const tblArmas = `
      ...
      <table class="table table-sm table-dark table-striped">
        <thead><tr><th>Equip.</th><th>Arma</th><th>Mano</th><th>Daño</th><th>Peso</th><th>Trasp.</th></tr></thead>
        <tbody>
          ${armasOrden.map(w => {
            return `
