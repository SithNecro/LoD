...
(function () {
    ...
    window.renderTalentosVentajasPreview = async function (slot) {
        ...
        ${items.length === 0 ? '<div class="TextosTalentos">— Ninguno —</div>' : items.map(it => `
          <div class="TextosTalentos p-2 mb-2">
            <div class="TextosVentajas" ><strong>${esc(it.tipo)}: ${esc(it.nombre)}</strong></div>
            <div >${it.efecto || ''}</div>
          </div>
        `).join('')}
      `;
            ...
            const html = `<div style="color:#fff;text-align:center;">${seccion('🧠 Talentos', pj.talentosVentajas.talentos) +
                seccion('🏅 Ventajas', pj.talentosVentajas.ventajas)}   </div>`;
            cont.innerHTML = html;
    ...
    window.openTalentosVentajasPopup = async function (slot) {
      ...
      <div id="tvRoot" class="hero-card p-2" style="max-height:${Math.min(window.innerHeight - 80, 700)}px;overflow:auto;">
        ...
        <h5>Aprende o Olvida Talentos y Ventajas</h5>
