...
window.tc_parseEnemyItem = function (itemEl) {
  ...
  const botinRaw    = getTextAfterStrong(itemEl, 'Botín');            // p.ej. "Antorchas (3).png" -> nombre visual
  const roturaTxt   = getTextAfterStrong(itemEl, 'Rotura del objeto'); // p.ej. "1"
  const categoriaTxt= getTextAfterStrong(itemEl, 'Categoria');         // "objeto" | "arma" | "armadura" (si lo has añadido)
  const durabTxt    = getTextAfterStrong(itemEl, 'Durabilidad');       // número (si lo has añadido)
  const cantidadTxt = getTextAfterStrong(itemEl, 'Cantidad');          // cadena con tirada/resultado si existe

  // En la zona verde:
  const tipoTxt     = getGreenValue(itemEl, 'Tipo');                    // p.ej. "Chaleco acolchado"
  ...
  const danioTxt    = getGreenValue(itemEl, 'Daño');                    // armas
  ...
  // ⚠️ El "(x)" en el nombre NO es cantidad, solo diferencia de PNG.
  //   - Limpiamos .png y el sufijo " (n)" SOLO para el nombre mostrado, NO para unidades.
  const nombreBase = botinRaw
    .replace(/\.png$/i, '')
    .replace(/\s*\(\d+\)\s*$/,'')    // quita "(3)" final en el nombre, pero no se usa como cantidad
    .trim();

  // Cantidad REAL: viene del campo "Cantidad:" del enemigo-item (último número de la cadena)
  let cantidad = 1;
  if (cantidadTxt) {
  ...
  if (categoria === 'objeto') {
    return {
      ...
      item: {
        ...
        cantidad: cantidad,         // ← de "Cantidad:", NUNCA del nombre
        peso: peso || 0,
        uso: efectoTxt || '',
