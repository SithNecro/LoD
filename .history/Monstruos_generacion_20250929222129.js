...
function renderCard(monster, category, armas, armaduras, hechizos, color = "#fff", vidas = null, save = true) {
    ...
    wrapper._vidas.forEach((vida, i) => {
        ...
        vidasGrid += `
            ...
            <div class="estado-herido" style="text-align:center; font-weight:bold; color:red; margin-top:3px;"></div>
  ...
  <span class="tipo-monstruo" 
    ...
    function actualizarVida(idx, nuevaVida) {
        ...
        if (wrapper._vidas[idx] <= monster.vida / 2 && wrapper._vidas[idx] > 0) {
            heridoEl.innerHTML = "💀 <span style='color:red; font-weight:bold;'>Herido: -1PA</span>";
        } else {
            heridoEl.innerHTML = "";
        }
        saveToLocalStorage();
