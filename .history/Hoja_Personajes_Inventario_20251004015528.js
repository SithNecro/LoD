...
window.openInventarioEditor = async function (slot) {
      ...
      <div id="invRoot" style="max-height:${modalHeight - 90}px; overflow:auto;">
        ...
        <div class="border rounded p-2 mb-3 hero-card">
          <div class="row g-2 align-items-end">
            ...
            <!-- Grupo Armadura (❗️cobertura ahora es select simple + acumulación en <p>) -->
    ...
    await Swal.fire({
        ...
        didOpen: () => {
            ...
            // 🔹 Cobertura acumulada para ARMADURA
            let armCobSel = [];
            ...
            const addCob     = () => {
                const val = elCobSel()?.value;
                if (!val) return;
                if (!armCobSel.includes(val)) armCobSel.push(val);
                paintCob();
            };
            ...
            // 👉 helper para resetear el select tipo y ocultar grupos
            const resetTipo = () => {
                elTipo.value = '';
                updateGroups();
                // limpiar coberturas si venías de Armadura
                clearCob();
                // limpiar campos básicos de cada grupo si quieres (opcional)
            };
            ...
            // Añadir ARMADURA (usa la lista acumulada armCobSel)
            ...
            const onInput = async (ev) => {
                ...
                if (updateItem(personaje.inventario.objetos, ['nombre','lugar','cantidad','peso','durabilidad','uso']) ||
                    updateItem(personaje.inventario.armaduras, ['armadura','defensa','especial','durabilidad','peso' /* cobertura ya no editable aquí */]) ||
