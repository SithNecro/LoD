...
function loadFromLocalStorage() {
    ...
    saved.forEach(data => {
        ...
        const vidasData = { length: data.vidas?.length || 0, _activados: data.activados || [] };
        const card = renderCard(monster, data.category, armas, armaduras, hechizos, data.color || "#fff", data.vidas || null, false);
        if (data.activados) card._activados = data.activados.slice(); card.dataset.info = JSON.stringify(data);
        container.appendChild(card);
        // Restaurar saqueos guardados
        if (data.saqueos) {
            const vidaItems = card.querySelectorAll(".vida-item");
            Object.entries(data.saqueos).forEach(([idx, tirada]) => {
                const item = vidaItems[idx];
                if (item) {
                    const btnSaqueo = item.querySelector(".btn-saquear");
                    const resultado = item.querySelector(".resultado-saqueo");
                    btnSaqueo.style.display = "none";
                    resultado.innerHTML = `<span style="color:gold;">Saqueado con un: ${tirada}</span>`;
    ...
    if (data.activados) {
    const vidaItems = card.querySelectorAll(".vida-item");
    data.activados.forEach((val, idx) => {
        const chk = vidaItems[idx]?.querySelector(".chk-activado");
        const lbl = vidaItems[idx]?.querySelector(".lbl-activado");
        if (chk && lbl) {
            chk.checked = val;
            if (val) {
                lbl.style.backgroundColor = "red";
                lbl.style.color = "white";
                chk.style.accentColor = "red";
            }
        }
    });
}
