...
window.openInventarioEditor = async function (slot) {
    ...
    await Swal.fire({
        ...
        didOpen: () => {
            ...
            // 👉 helper para resetear el select y ocultar grupos tras "Añadir"
            const resetTipo = () => {
                elTipo.value = '';
                updateGroups(); // oculta todos
                // opcional: limpiar campos del grupo que quedó visible antes
                // (no imprescindible para tu petición)
            };

            elTipo.addEventListener('change', updateGroups);
            // Inicio en placeholder → todo oculto
            elTipo.value = '';
            updateGroups();
            ...
            document.getElementById('btnAddObj').addEventListener('click', async () => {
                ...
                resetTipo(); // 👈 vuelve al placeholder y oculta grupos
            });
      ...
      <table class="table table-sm align-middle ">
        ...
        ${personaje.inventario.objetos.map(o => `
          <tr data-itemid="${o.id}">
            ...
            <td><p class="form-control form-control-sm" name="nombre" value="${o.nombre || ''}">${o.nombre || ''}</td>
            <td><p class="form-control form-control-sm" name="lugar" value="${o.lugar || ''}">${o.lugar || ''}</td>
            <td><p class="form-control form-control-sm" type="number" min="0" name="cantidad" value="${o.cantidad ?? 0}">${o.cantidad ?? 0}</td>
            <td><p class="form-control form-control-sm" type="number" step="0.1" min="0" name="peso" value="${o.peso ?? 0}">${o.peso ?? 0}</td>
            <td><p class="form-select form-select-sm" name="durabilidad">${o.durabilidad ?? 0}</td>
            <td><p class="form-control form-control-sm" name="uso" value="${o.uso || ''}">${o.uso || ''}</td>
            <td>
