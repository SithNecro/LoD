...
// Seleccionas un legendario desde el select
function cambiarImagenSeleccionada() {
  ...
  // Render del bloque con botón + select de héroes
...
async function renderLegendarioDetalle(filenamePng) {
  ...
  // 1) Cargar efecto desde tu JSON (stats_tesoros_legendarios.json)
  let efecto = '';
  try {
  ...
  // 3) Controles: select de héroes + botón
  ...
  if (heroes.length === 0) {
    ...
    sel.style.cssText = 'width:auto;display:inline-block;margin-right:6px; background-image:url(\\'img/interface/select_arrow.png\\'); background-repeat:no-repeat; background-position:right 10px center; background-size:20px; padding-right:35px; appearance:none;';
    ...
    btn.addEventListener('click', async () => {
      ...
      try {
        const obj = {
          id: Date.now(),
          nombre: nombreLimpio, // Nombre del legendario
          cantidad: 1,
          peso: 0,
